...
.css_button:active {
	...
	$query1 = mysql_query('SELECT `player_id` , `player_name` ,`country` ,`type` FROM '.$match.' ORDER BY `player_id`');
			while($row1=mysql_fetch_assoc($query1)){ 
				//echo "<li class='squad'><button class='css_button'> { $row1['player_name'] }</button></li>";
			
			?>
			<li class='squad'><button class='css_button' id=<?php echo $row1['player_id']; ?> > <?php echo $row1['player_name']; ?> </button></li>
			<br/>
		<?php }?>
